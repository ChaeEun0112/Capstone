...
// 최신 데이터를 정렬하여 반환하는 API
server.get("/ultrasonic", async (req, res) => {
    try {
        const data = await Ultrasonic.find().sort({ created_at: -1 }); // created_at 기준 내림차순 정렬
        res.status(200).json(data); // JSON 형식으로 클라이언트에 응답
    } catch (error) {
            ...
            if (obj.distance !== undefined && obj.status) {
                ...
                await ultrasonicData
                    ...
                    .then(() => {
                        ...
                        // **저장 후 최신 데이터 클라이언트로 전송**
                        Ultrasonic.find().sort({ created_at: -1 }).limit(10)
                            .then(data => io.emit('ultrasonicData', data));
                    })
                    .catch((err) => {
